...
local function change_python_path(bufnr)
    vim.ui.input({
        prompt = "Nhập đường dẫn đến Python (ví dụ: ~/.python_envs/my_env/bin/python): ",
        ...
        if input and input ~= "" then
            ...
            lspconfig.pyright.setup({
                ...
                on_attach = function(client, bufnr)
                    ...
                    -- Sử dụng client để tránh cảnh báo "Unused local"
                    vim.notify("Pyright client ID: " .. client.id, vim.log.levels.INFO)
                end,
            })
...
local function python_keymaps(bufnr)
    ...
    vim.keymap.set("n", "<leader>Pp", function() change_python_path(bufnr) end,
        vim.tbl_extend("force", opts, { desc = "Đổi đường dẫn Python" }))
end
